...
export class BudgetsService {
  ...
  async create(
    userId: string,
    data: { name: string; value: number; period?: string; categoryIds: string[] },
  ): Promise<Budget> {
    return sequelize.transaction(async (t) => {
      const budget = await Budget.create(
        { name: data.name, value: data.value, period: data.period, userId },
        { transaction: t },
      );

      const categories = await Category.findAll({
        where: { id: data.categoryIds, userId, type: 'expense' },
        transaction: t,
      });

      if (categories.length !== data.categoryIds.length) {
        throw new ForbiddenError('Budgets can only include expense categories');
      }
  ...
  async update(
    id: string,
    userId: string,
    data: { name?: string; value?: number; categoryIds?: string[] },
  ): Promise<Budget> {
    return sequelize.transaction(async (t) => {
  ...
  async getStats(userId: string, year: number, month: number): Promise<BudgetStats> {
    ...
    const budgetStats: BudgetStat[] = budgets.map((budget) => {
      ...
      const allocated = Number(budget.value);
